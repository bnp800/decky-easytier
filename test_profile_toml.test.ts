...
test('default profile is valid TOML with safe native defaults', () => {
  const parsed: any = TOML.parse(defaultProfileToml());
...
test('form round trip preserves unknown advanced values', () => {
  ...
  const parsed: any = TOML.parse(writeCommonFields(source, fields));
...
test('proxy mapping and list fields are normalized', () => {
  ...
  const parsed: any = TOML.parse(writeCommonFields(defaultProfileToml(), fields));
